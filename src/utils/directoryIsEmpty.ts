...
export const defaultWhiteListedFiles = [];
...
export default async function isDirectoryEmpty(
  aPath: string,
  whiteListedFiles: string[] = defaultWhiteListedFiles
) {
  ...
  const nonWhiteListedFiles = files.filter(
    (file) => whiteListedFiles.indexOf(file) === -1
  );
